...
from utils.utils import get_elapse_time, load_and_cache_multi_gen_data
...
def eval_bleu(args, eval_data, eval_examples, model, tokenizer, split_tag, cur_task, criteria, logger):
    ...
    if task == 'defect':
        ...
        dev_accs = []
        predictions = []
        res_dir = os.path.join(args.run_output_dir, 'prediction', cur_task)
...
def main():
    ...
    fa = open(os.path.join(args.log_dir, 'summary.log'), 'a+')

    fa_dict = {}
    if not args.no_train:
        if args.local_rank in [-1, 0] and args.data_num == -1:
            summary_fn = os.path.join(args.output_dir,'tensorboard', args.project_name, args.name)
        ...
        for cur_task in all_tasks:
            fa_dict[cur_task] = open(os.path.join(args.log_dir, f'{cur_task}.log'), 'a+')

        args.train_batch_size = args.train_batch_size[0]
        args.eval_batch_size = args.eval_batch_size[0]
        effective_global_batch_size = (args.n_gpu * args.train_batch_size * args.gradient_accumulation_steps)
        global_batch_size = effective_global_batch_size // args.gradient_accumulation_steps
        factor = effective_global_batch_size / args.train_batch_size

        # TODOS: fixed batch size, might be an issue when working with different tasks with different max_lengths.
        num_step_per_epoch = dict()
        ...
        for train_data, cur_task in zip(train_data_list, all_tasks):
            ...
            num_step_per_epoch[cur_task.split('_')[0]] = len(train_dataloader)
            train_dataloader_dict[cur_task] = cycle(train_dataloader)

        # Infer max_steps from the number of epochs to control for budget and scale depending on the number of gpus.
        task_epochs = get_task_epochs()
        args.max_steps = 0
        for k,v in num_step_per_epoch.items():
            args.max_steps += task_epochs[k] * v

        lr = args.learning_rate[0] * math.sqrt(factor) if factor > 1 else args.learning_rate[0]
        args.max_steps = int(args.max_steps / args.n_gpu)
        args.save_steps = int(args.max_steps / args.num_saves)
        logger.info(f"\t Tasks: {all_tasks}\t")
        logger.info(f"Inferred LR as {lr:.4f} based on sqrt scaling rule (BS scale {factor:.2f}x).")
        logger.info(f"Scaled Max Step from {args.max_steps * factor} to {args.max_steps} using factor {factor:.2f}).")
        logger.info(f"Global batch size: {global_batch_size}, save steps: {args.save_steps}")
            ...
            if nb_tr_steps % args.gradient_accumulation_steps == 0:
                ...
                if not args.no_eval and args.local_rank in [-1, 0] \
                    ...
                    if args.data_num == -1 and args.save_last_checkpoints:
                        ...
                        output_model_file = os.path.join(args.checkpoint_dir, "last.bin")
                        torch.save(model_to_save.state_dict(), output_model_file)
                        logger.info("Save the last model into %s", output_model_file)
                    if global_step % 100000 == 0:
                        step_tag = '{}00k'.format(global_step // 100000)
                        model_to_save = model.module if hasattr(model, 'module') else model
                        output_model_file = os.path.join(args.checkpoint_dir, f"step-{step_tag}.bin")
                        torch.save(model_to_save.state_dict(), output_model_file)
                        logger.info("Save the last model into %s", output_model_file)
                    ...
                    for cur_task in eval_examples_data_dict.keys():
                        ...
                        if eval_loss < best_loss[cur_task]:
                            ...
                            if args.data_num == -1 or args.always_save_model:
                                model_to_save = model.module if hasattr(model, 'module') else model
                                output_model_file = os.path.join(args.checkpoint_dir, f"best-ppl-{cur_task}.bin")
                                torch.save(model_to_save.state_dict(), output_model_file)
                    ...
                    if not args.no_eval_bleu:
                        ...
                        for cur_task in eval_examples_data_dict.keys():
                            ...
                            result = eval_bleu(args, eval_data, eval_examples, model, tokenizer, 'dev', cur_task,
                                               criteria='e{}'.format(global_step), logger=logger)
                            ...
                            if dev_bleu_em > best_bleu_em[cur_task]:
                                ...
                                if args.data_num == -1 or args.always_save_model:
                                    model_to_save = model.module if hasattr(model, 'module') else model
                                    output_model_file = os.path.join(args.checkpoint_dir, f"best-bleu-{cur_task}.bin")
                                    torch.save(model_to_save.state_dict(), output_model_file)
    ...
    for cur_task in all_tasks:
        ...
        for criteria in ['best-bleu']:#, 'best-ppl', 'last']:
            file = os.path.join(args.checkpoint_dir, f'{criteria}-{cur_task}.bin')
            ...
            result = eval_bleu(args, eval_data, eval_examples, model, tokenizer, 'test', cur_task, criteria, logger)
            test_bleu, test_em = result['bleu'], result['em']
            test_codebleu = result['codebleu'] if 'codebleu' in result else 0
            result_str = "[%s %s] bleu: %.2f, em: %.4f, codebleu: %.4f\n" % (
